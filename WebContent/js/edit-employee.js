angular.module('edit-employee', ['formly', 'formlyBootstrap']).constant('formlyExampleApiCheck', apiCheck()).config(function config(formlyConfigProvider, formlyExampleApiCheck) {
    // set templates here
      ...
      defaultOptions: function matchFieldDefaultOptions(options) {
		...
		$rootScope.buttonText = (id > 0) ? 'Update' : 'Add';
			...
			departmentService.list().then(function (status) {
				var deps = status.data;
				angular.forEach(deps, function(dep) {
					$scope.departments.push({name: dep.name, value: dep.id});
				});
			});
			var vm = this;
			$scope.vm = vm;
			...
			function onSubmit() {
				if (id > 0) {
					...
					employeeService.insert(vm.model);
				}
