...
public class LeavesettingsResourceTest extends ResourceTest {
	...
	public void testGetLeavesetting() {
		Response response = target("leavesetting").path("1")
	...
	public void testGetLeavesettingWithNotFoundException() {
		Response response = target("leavesetting").path("2")
				.register(JacksonFeature.class).request().get();
	...
	public void testSaveLeavesetting() {
		...
		Response response = target("leavesetting")
		...
		assertEquals(
				transfer.getType().getName() + "_" + leavesetting.getYear()
						+ "_" + leavesetting.getDays(), transfer.getName());
	}
